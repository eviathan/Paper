...
export function activate(context: vscode.ExtensionContext) {
    ...
    const preOpenGenerated = (csxPath: string) => {
    ...
    context.subscriptions.push(
        vscode.languages.registerColorProvider(
            { scheme: 'file', language: 'csss' },
            {
                ...
                provideColorPresentations(color: vscode.Color, ctx: { document: vscode.TextDocument; range: vscode.Range }) {
...
class CSXHoverProvider implements vscode.HoverProvider {
    provideHover(document: vscode.TextDocument, position: vscode.Position): vscode.Hover | null {
        ...
        const word = document.getText(wordRange);
        if (!PAPER_ELEMENTS.includes(word)) return null;
        ...
        if (!/<\/?$/.test(before.trimEnd())) return null;
...
function parseCsssColors(document: vscode.TextDocument): vscode.ColorInformation[] {
    const text   = document.getText();
...
function formatCsss(text: string, tabSize: number, insertSpaces: boolean): string {
 ...
 */
async function delegateHoverToGenerated(
    document: vscode.TextDocument,
    position: vscode.Position
    ...
    try {
        ...
        const hovers = await vscode.commands.executeCommand<vscode.Hover[]>(
            'vscode.executeHoverProvider', generatedUri, mappedPos
        );
 ...
 */
function mapCsxToGeneratedPosition(
    csxLines: string[],
    genLines: string[],
    position: vscode.Position
    ...
    for (let i = 0; i < genLines.length; i++) {
        if (/public static UINode \w+\(Props props\)/.test(genLines[i])) {
            ...
            for (let j = i + 1; j < Math.min(i + 3, genLines.length); j++) {
                if (genLines[j].trim() === '{') { genPreambleStart = j + 1; break; }
            }
            break;
        }
    }

    if (csxPreambleStart < 0 || genPreambleStart < 0) return null;
    ...
    let injectedLines = 0;
    while (
        genPreambleStart + injectedLines < genLines.length &&
    ...
    if (line === csxFuncDeclLine) {
        ...
        if (genMethodSigLine < 0) return null;
    ...
    if (line < csxPreambleStart) return null; // @import lines, blank lines before function
    ...
    for (let i = csxPreambleStart; i < csxLines.length; i++) {
        ...
        if (ws < csxBaseIndent) csxBaseIndent = ws;
    }
