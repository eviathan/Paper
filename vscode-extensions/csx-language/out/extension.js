 ...
 */
function csxElementHover(document, position) {
    const lineText = document.lineAt(position).text;
 ...
 */
function csxAttrHover(document, position) {
    const lineText = document.lineAt(position).text;
    const word = getWordAt(lineText, position.character);
...
function findContainingElement(document, position) {
    for (let i = position.line; i >= Math.max(0, position.line - 15); i--) {
        ...
        if (i < position.line && /^\s*>/.test(text))
            break;
    }
    return null;
}
function elementDocHover(name, doc) {
    const md = new vscode.MarkdownString();
    md.isTrusted = true;
...
function mapCsxToGeneratedPosition(csxLines, genLines, position) {
    ...
    for (let i = genBodyStart; i < genLines.length; i++) {
        ...
        if (csxPrefix.startsWith(genTrimmed.slice(0, genPrefixLen)) ||
